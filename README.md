...
| <a name="input_subnets"></a> [subnets](#input\_subnets) | For each subnet, create an object that contain fields | <pre>map(object({<br>    subnet_name                                    = string<br>    subnet_address_prefix                          = list(string)<br>    nsg_name                                       = string<br>    enforce_private_link_endpoint_network_policies = string<br>    service_endpoints                              = list(string)<br>    route_table_name                               = string<br>    disable_bgp_route_propagation                  = bool<br>  }))</pre> | `{}` | no |
